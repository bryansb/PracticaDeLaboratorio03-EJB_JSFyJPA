...
public class JPAGenericDAO<T, ID> implements GenericDAO<T, ID> {
	...
	this.em = Persistence.createEntityManagerFactory("Practica03")
		.createEntityManager();
    }
	...
	for (int i = 0; i < attributes.length; i++) {
	    ...
	    predicate = criteriaBuilder.and(predicate,
		    getSig(criteriaBuilder, path.as(String.class), values[i]));
	}
	...
	Predicate predicate = criteriaBuilder.conjunction();
	for (int i = 0; i < attributes.length; i++) {
	    Predicate sig = criteriaBuilder
		    .like(root.get(attributes[i]).as(String.class), values[i]);
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    public List<T> findByJoin(String[] classes, String[][] attributes,
	    String[] values, String order, int index, int size,
	    boolean isDistinct) {
	...
	// NOTA: Estana .isBlank()
	if (classes[k].isEmpty()) {
	    sig = getSig(criteriaBuilder,
		    root.get(attributes[k][0]).as(String.class), values[k]);
	    predicate = criteriaBuilder.and(predicate, sig);
	    k++;
	}
	...
	if (!attributes[k][0].isEmpty()) {
	    sig = getSig(criteriaBuilder,
		    join.get(attributes[k][0]).as(String.class), values[k]);
	    predicate = criteriaBuilder.and(predicate, sig);
	}
	...
	for (int i = k; i < classes.length; i++) {
		...
		if (!attributes[i][j].isEmpty()) {
		    sig = getSig(criteriaBuilder,
			    join.get(attributes[i][j]).as(String.class),
			    values[i]);
		    predicate = criteriaBuilder.and(predicate, sig);
		}
	    }
